...
make(const Point& source, const Point& target,
   ...
   _leftSideNormal = cross(_segment.getTangent(), leftNormal);
   _rightSideNormal = cross(rightNormal, _segment.getTangent());

   const Number vol = dot
                      (cross(leftNormal, rightNormal),
                       _segment.getTangent());
   if (vol > 0) {
      _signOfDistance = 1;
   }
...
isValid() const {
   ...
   if (std::abs(magnitude(_leftFaceNormal) - 1) > eps) {
      return false;
   }
   if (std::abs(magnitude(_rightFaceNormal) - 1) > eps) {
      return false;
   }

   if (std::abs(magnitude(_leftSideNormal) - 1) > eps) {
      return false;
   }
   if (std::abs(magnitude(_rightSideNormal) - 1) > eps) {
      return false;
   }
...
buildCharacteristicPolyhedron(Polyhedron* polyhedron,
   ...
   Number cosTheta = dot(getLeftFaceNormal(),
                         getRightFaceNormal());
...
isInside(const Point& pt, const Number delta) const {
   ...
   if (dot(pt - getSource(), _leftSideNormal) >= - delta &&
   dot(pt - getSource(), _rightSideNormal) >= - delta) {
      // If the point is between the top and bottom of the wedge.
      Number proj = dot(pt - getSource(), getTangent());
