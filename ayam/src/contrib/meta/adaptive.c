...
void
meta_midpoint(int e, meta_vertex *v0, meta_vertex *v1, meta_vertex *m, meta_world *w)
{
   meta_vertex mm;
   double va;
...
void
meta_tri_adapt(meta_vertex *v0, meta_vertex *v1, meta_vertex *v2, int k, meta_world *w)
{
   int e1,e2,e3;
   ...
   if((e1&&e2&&e3)||(k==0))
   {
	...
	if (w->currentnumpoly >= (w->maxpoly))
 	    	{
			 w->vertex =  realloc (w->vertex,  sizeof (double) * 3 * 3 * (w->maxpoly + 10000 + 20));
			 w->nvertex =  realloc (w->nvertex,  sizeof (double) * 3 * 3 * (w->maxpoly + 10000 + 20));
	     	 w->maxpoly += 10000;
		}
...
void
meta_intersect(meta_gridcell *cube,int k0, int k1, meta_vertex *i, meta_world *w)
{
   double t,tt;
   meta_vertex m;
   ...
   *i = meta_project_s(w->step,m,meta_calcall(m.p.x, m.p.y, m.p.z, w),MAX_REC, w);
...
void
meta_simplex(meta_gridcell *cube, int k0, int k1, int k2, int k3, meta_world *w)
{
   double v0,v1,v2,v3;
   ...
   if(v0 < 0)
     if(v1 < 0)
       if(v2 < 0)
	    if(v3 < 0) ; /* no intersection*/
	    else meta_tri(cube,k3,k2,k1,k0,w);
	  else
	   if(v3 < 0) meta_tri(cube,k2,k0,k1,k3,w);
	   else meta_quad(cube,k2,k3,k0,k1,w);
	else
	 if(v2 < 0)
	   if(v3 < 0) meta_tri(cube,k1,k3,k2,k0,w);
	   else meta_quad(cube,k1,k3,k2,k0,w);
	 else
	   if(v3 < 0) meta_quad(cube,k1,k2,k0,k3,w);
	   else meta_tri(cube,k0,k3,k2,k1,w);
   else
     if(v1 < 0)
	  if(v2 < 0)
	    if(v3 < 0) meta_tri(cube,k0,k1,k2,k3,w);
	    else		meta_quad(cube,k0,k3,k1,k2,w);
	  else 
	    if(v3 < 0) meta_quad(cube,k0,k2,k3,k1,w);
	    else		meta_tri(cube,k1,k3,k0,k2,w);
	else
	  if(v2 < 0)
	    if(v3 < 0) meta_quad(cube,k0,k1,k2,k3,w);
	    else		meta_tri(cube,k2,k3,k1,k0,w);
	  else
	    if(v3 < 0) meta_tri(cube,k3,k0,k1,k2,w);
	    else ;
