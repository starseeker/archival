   ...
   if (! parser.areArgumentsEmpty()) {
      ...
      if (!(areUsingAsciiOutput || areUsingBinaryOutput)) {
         ...
         for (std::size_t n = 0; n != numberOfGrids; ++n) {
            ...
            const Number* iter_end = iter + product(gridExtents[n]);
            ...
            if (areComputingGradientOfDistance) {
               ...
               const Point* iter_end = iter + product(gridExtents[n]);
               for (; iter != iter_end; ++iter) {
                  file << *iter << '\n';
               }
               file << "</DataArray>\n";
            ...
            if (areComputingClosestFace) {
               ...
               const std::size_t* iter_end = iter + product(gridExtents[n]);
               for (; iter != iter_end; ++iter) {
                  file << *iter << "\n";
               }
               file << "</DataArray>\n";
